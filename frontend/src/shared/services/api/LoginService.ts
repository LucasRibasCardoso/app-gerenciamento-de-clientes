	...
	try {
		...
		if (error.response) {
			if (isValidationError(error.response.data)) {
				return {
					message:
						error.response.data.message || "Erro de validação nos campos enviados.",
		...
		return {
			message:
				"Ocorreu um erro inesperado ao tentar realizar o login. Verifique sua conexão e tente novamente.",
			statusCode: 500,
		} as GenericError;
	}
};
